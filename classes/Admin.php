...
class Admin{
    ...
    public function save_campaign_post(){
        ...
        $campaign_data['message'] = wp_kses_post($_POST['message']);
    ...
    public function sync_users(){
        ...
        $wp_users = $wpdb->get_results("select * from {$wpdb->prefix}users WHERE ID > {$lastSyncWPUserID} ");
    ...
    public function lp_test_campaign(){
        ...
        if (isset($_GET['campaign_id'])){
            ...
            $campaign = $wpdb->get_row("select * from {$wpdb->prefix}lp_campaigns WHERE id = {$campaign_id} limit 1; ");
