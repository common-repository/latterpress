...
class Rewrite_URL{
    ...
    function template_include_view_campaign( $template ) {
        ...
        $campaign = $wpdb->get_row("select * from {$wpdb->prefix}lp_campaigns WHERE slug = '{$email_token}' ");
        if (empty($campaign)){
            return $template;
        }
        echo stripslashes($campaign->message);
    ...
    function template_include_unsubscribe( $template ) {
        ...
        $user = $wpdb->get_row("select * from {$wpdb->prefix}lp_subscribers WHERE token = '{$user_token}' ");
        if (empty($user)){
            //return $template;
        }
