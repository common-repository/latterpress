...
class Subscribers_List extends \letterpress\LP_List_Table {
    ...
    function process_bulk_action() {
        ...
        if( 'delete'===$this->current_action() ) {
            if ($subscriber){
                $wpdb->query("DELETE from {$wpdb->prefix}lp_subscribers WHERE id IN ({$subscriber});");
            }
        }
        if( 'block'===$this->current_action() ) {
            if ($subscriber){
                $wpdb->query("UPDATE {$wpdb->prefix}lp_subscribers SET status='B' WHERE id IN({$subscriber}) ;");
            }
        }
        if( 'subscribe'===$this->current_action() ) {
            if ($subscriber){
                $wpdb->query("UPDATE {$wpdb->prefix}lp_subscribers SET status='S' WHERE id IN({$subscriber}) ;");
            }
        }
        if( 'unsubscribe'===$this->current_action() ) {
            if ($subscriber){
                $wpdb->query("UPDATE {$wpdb->prefix}lp_subscribers SET status='U' WHERE id IN({$subscriber}) ;");
            }
        }
    }
    ...
    function prepare_items() {
        ...
        if (isset($_POST['s'])){
            ...
            $subscriberQuery .= " WHERE name LIKE '%{$searchTerm}%' OR email LIKE '%{$searchTerm}%' ";
        }

        $subscribers = $wpdb->get_results($subscriberQuery." ORDER BY id DESC limit {$offset}, {$per_page} ;", ARRAY_A );
